...
namespace Alphes_Project_0._3
{
    internal class Program
    {
        ...
        static string[,] allOrders = new string[100, 3];
        ...
        static void AddOders()
        {
            ...
            {
                if (orderCount >= allOrders.GetLength(0))
                {
                    Console.WriteLine("Order limit reached. Cannot add more orders.");
                    Console.WriteLine("( Press Enter to continue )");
                    Console.ReadLine();
                    break;
                }
                ...
                for (int i = 0; i < toppingCount; i++)
                {
                    ...
                    {
                        Console.Write($"[{i + 1}] Enter topping : ");
                    ...
                    selectedToppings[i] = toppingName;
                    Console.WriteLine($"---> Added '{toppingName}'");
                }

                for (int j = 0; j < 3; j++)
                {
                    allOrders[orderCount, j] = selectedToppings[j];
                }
        ...
        static void ShowCart()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < orderCount; i++)
                {
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        if (!string.IsNullOrEmpty(allOrders[i, j]))
                        {
                            toppingCount++;
                        }
                    }
                    ...
                    int toppingsPrice = toppingCount * PriceTopping;
                    totalPrice += omelettePrice + toppingsPrice;

                    Console.WriteLine($"[{i + 1}] Omelette x1             {omelettePrice} THB");
                    for (int j = 0; j < 3; j++)
                    {
                        if (!string.IsNullOrEmpty(allOrders[i, j]))
                        {
                            Console.WriteLine($"     - {allOrders[i, j],-25}{PriceTopping} THB");
                        }
                    }
                }
        ...
        static void CheckOut()
        {
            ...
            for (int i = 0; i < orderCount; i++)
            {
                ...
                totalPrice += PriceOmelette + toppingCount * PriceTopping;
            }
